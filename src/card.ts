  ...
  async renderReady(input: { report: ReadyReport; realm: string }): Promise<Buffer> {
    return this.withBrowser(async (browser) => {
      ...
      const ready = ROLE_TARGETS.reduce((total, group) => total + Math.min(group.target, members.filter((member) => (
        member.signup.reportedRole === group.role && member.summary.score >= RAID_READY_GEAR_SCORE
      )).length), 0);
      ...
      const roleSections = ROLE_TARGETS.map((group) => {
        ...
        return `<section class="role-section"><div class="role-heading"><span>${group.label}</span><b class="${state}">${signups.length}/${group.target}</b></div><div class="roster-head"><span>Signup name</span><span>Event spec</span><span>iLvl</span><span>GS</span></div>${signups.length ? rosterRows(signups) : `<div class="empty-role">No ${group.label.toLowerCase()} signed</div>`}</section>`;
      });
      const unassigned = input.report.activeSignups.filter((signup) => !signup.reportedRole).sort(sortByGearScore);
      if (unassigned.length) roleSections.push(`<section class="role-section unassigned"><div class="role-heading"><span>Role not selected</span><b class="short">${unassigned.length}</b></div><div class="roster-head"><span>Signup name</span><span>Event spec</span><span>iLvl</span><span>GS</span></div>${rosterRows(unassigned)}</section>`);
