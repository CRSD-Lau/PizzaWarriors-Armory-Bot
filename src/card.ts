...
export class ArmoryCardRenderer {
  ...
  async renderReady(input: { report: ReadyReport; realm: string }): Promise<Buffer> {
    return this.withBrowser(async (browser) => {
      ...
      const ready = members.filter((member) => member.summary.score >= RAID_READY_GEAR_SCORE).length;
      const formatSpec = (value: string) => value.replace(/_/g, " ").replace(/(?<=\D)1$/, "");
      const rosterRows = (list: typeof members) => list.map((member) => `<div class="ready-row"><span class="ready-name" style="color:${classColors[member.className ?? ""] ?? "#f1f3f7"}">${escapeHtml(member.signup.displayName)}</span><span class="ready-spec">${escapeHtml(formatSpec(member.specName ?? "No event spec selected"))}</span><span class="ready-ilvl">i${member.summary.averageItemLevel}</span><span class="ready-gs ${member.summary.score >= RAID_READY_GEAR_SCORE ? "ready" : "review"}">${member.summary.score.toLocaleString()}</span></div>`).join("");
