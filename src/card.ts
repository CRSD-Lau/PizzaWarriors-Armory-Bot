...
import { activeSignupsOutsideCore, type CoreRosterAudit, type CoreRosterStatus } from "./core-roster.js";
...
import { screenshotCard } from "./card-rendering.js";
...
export class ArmoryCardRenderer {
  ...
  async render(input: CardInput): Promise<Buffer> {
    ...
    return screenshotCard(browser, html, { width: 920, height: 1_400 });
    });
  }
    ...
    return this.withBrowser(async (browser) => {
      ...
      const readinessSub = notReady === 0
        ...
        : openSpots > 0
          ...
          : `${notReady} required slot${notReady === 1 ? "" : "s"} unfilled or gear/prep not verified`;
      ...
      const preparationIssues = members.filter((member) => member.preparation.status !== "complete").map((member) => {
        const details = member.freshness?.stale
          ? `Cached gear (${Math.max(1, Math.round((Date.now() - member.freshness.fetchedAt) / 60_000))} min old) · live preparation not verified`
          : member.preparation.status === "unverified"
      ...
      const preparationSummary = preparationIssues.length ? `<div class="prep-summary"><div class="prep-summary-title">Preparation needs review · ${preparationIssues.length}</div>${preparationIssues.join("")}</div>` : "";
      ...
      return screenshotCard(browser, html, { width: 920, height: 1_600 });
    });
  }
  ...
  async renderAttendance(input: { history: CoreAttendanceHistory }): Promise<Buffer> {
    return this.withBrowser(async (browser) => {
      ...
      return screenshotCard(browser, html, { width: cardWidth + 48, height: 1_600 });
    });
  }
  ...
  async renderRoster(input: { roster: GuildRoster; page: number; pageSize?: number }): Promise<Buffer> {
    ...
    return screenshotCard(browser, html, { width: 920, height: 1_100 });
    });
  }
}
