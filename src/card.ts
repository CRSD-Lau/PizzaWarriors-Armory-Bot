...
export class ArmoryCardRenderer {
  ...
  async renderReady(input: { report: ReadyReport; realm: string }): Promise<Buffer> {
    return this.withBrowser(async (browser) => {
      ...
      if (unassigned.length) roleSections.push(`<section class="role-section unassigned"><div class="role-heading"><span>Role not selected</span><b class="short">${unassigned.length}</b></div><div class="roster-head"><span>Signup name</span><span>Event spec</span><span>iLvl</span><span>GS</span><span>Prep</span></div>${rosterRows(unassigned)}</section>`);
