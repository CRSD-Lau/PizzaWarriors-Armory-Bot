...
export class ArmoryCardRenderer {
  ...
  async renderReady(input: { report: ReadyReport; realm: string }): Promise<Buffer> {
    return this.withBrowser(async (browser) => {
      ...
      </style></head><body><main class="card"><header class="brand"><img src="${logo}" alt="PizzaWarriors"><span>PizzaWarriors Raid Readiness</span></header><div class="title">${escapeHtml(input.report.eventTitle)}</div><div class="realm">${escapeHtml(input.realm)} · Raid-Helper signup selections</div><div class="stats"><div class="stat"><span class="label">Signed</span><span class="value">${signed.length}</span><span class="sub">${late.length ? `${late.length} late attendee${late.length === 1 ? "" : "s"}` : "Active roster"}</span></div><div class="stat"><span class="label">Average GS</span><span class="value">${average.toLocaleString()}</span><span class="sub">${members.length}/${input.report.activeSignups.length} gear profiles loaded</span></div><div class="stat"><span class="label">Raid ready</span><span class="value">${ready}/${members.length}</span><span class="sub">5,800+ GearScore</span></div></div><div class="attendance"><div class="attendance-count"><span>Signed</span><b>${signed.length}</b></div><div class="attendance-count tentative"><span>Tentative</span><b>${attendance("Tentative").length}</b></div><div class="attendance-count bench"><span>Bench</span><b>${attendance("Bench").length}</b></div><div class="attendance-count absent"><span>Absent</span><b>${attendance("Absent").length}</b></div></div>${members.length ? `<div class="roster"><section><div class="roster-head"><span>Signup name</span><span>Event spec</span><span>iLvl</span><span>GS</span></div>${rosterRows(members.slice(0, midpoint))}</section><section><div class="roster-head"><span>Signup name</span><span>Event spec</span><span>iLvl</span><span>GS</span></div>${rosterRows(members.slice(midpoint))}</section></div>` : `<div class="empty">No signed or late attendees have an available Warmane gear profile yet.</div>`}<div class="attendance-notes">${statusNames("Late")}${statusNames("Tentative")}${statusNames("Bench")}${statusNames("Absent")}</div>${unresolved}<div class="footer"><strong>PizzaWarriors Raid Readiness</strong> · Event-selected specs · Raid-Helper live event ${escapeHtml(input.report.eventId)}</div></main></body></html>`;
