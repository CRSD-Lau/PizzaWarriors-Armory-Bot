...
export class ArmoryCardRenderer {
  ...
  async renderUpgrade(input: { name: string; realm: string; className: string; specName: string; profile: UpgradeProfile; items: GearItem[]; portrait?: Buffer }): Promise<Buffer> {
    ...
    const targetRows = (list: UpgradeTarget[]) => list.map((target) => {
      const hasTarget = equipped.has(target.id);
    ...
    const targetContent = targets.length
      ? `<div class="grid"><section><h2>Priority targets</h2>${targetRows(targets.slice(0, midpoint))}</section><section><h2>Priority targets</h2>${targetRows(targets.slice(midpoint))}</section></div><div class="note">This guide-linked target matrix is a practical ICC/Ruby Sanctum path, not an automatic best-in-slot claim. Caps, set bonuses, raid access, and officer review still apply.</div>`
    ...
    </style></head><body><main class="card"><header class="brand"><img src="${logo}" alt="PizzaWarriors"><span>PizzaWarriors Upgrade Advisor</span></header><div class="identity"><div><div class="name">${escapeHtml(input.name)}</div><div class="realm">${escapeHtml(input.realm)} · ${escapeHtml(input.specName)} ${escapeHtml(input.className)}</div></div>${portrait ? `<img class="portrait" src="${portrait}" alt="">` : ""}</div><div class="stats"><div class="stat"><span class="label">Guide targets</span><span class="value">${targets.length || "—"}</span><span class="sub">${targets.length ? "Full target matrix" : "Source loading"}</span></div><div class="stat"><span class="label">Already equipped</span><span class="value">${targets.length ? targets.filter((target) => equipped.has(target.id)).length : "—"}</span><span class="sub">${targets.length ? "Exact item matches" : "No target list yet"}</span></div><div class="stat"><span class="label">Profile status</span><span class="value">Guide linked</span><span class="sub">Officer-adjustable</span></div></div>${targetContent}<div class="footer"><strong>PizzaWarriors Upgrade Advisor</strong> · ${escapeHtml(input.profile.sources[0]?.title ?? "Warmane source")}</div></main></body></html>`;
